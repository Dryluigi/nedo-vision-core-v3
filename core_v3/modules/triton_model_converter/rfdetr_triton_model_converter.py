...
class RfdetrTritonModelConverter(TritonModelConverterInterface):
    ...
    def is_ready(self, ai_model: AIModelEntity) -> bool:
        ...
        return all(p.exists() and p.stat().st_size > 0 for p in required_paths)
    ...
    def prepare(self, ai_model: AIModelEntity):
        ...
        self._run(
            [
                "trtexec",
                f"--onnx={paths['onnx_path']}",
                f"--saveEngine={paths['plan_path']}",
        ...
        paths["trt_config_path"].write_text(
        ...
        paths["ensemble_config_path"].write_text(
        ...
        paths["infer_config_path"].write_text(
